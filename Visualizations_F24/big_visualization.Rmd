...
c <- 1
for (n in x) {
  m0 <- read.table(file = paste("deg_sisA/plus/canonical_tagteam/csv/", n, "_genes_plus_500_upstream.csv", sep= ""), sep = '\t', header = TRUE)
...
#minus
for (n in x) {
  m0 <- read.table(file = paste("deg_sisA/minus/canonical_tagteam/csv/", n, "_genes_minus_500_upstream.csv", sep= ""), sep = '\t', header = TRUE)
  m0$seqID[0:length(m0$seqID)] <- n
  colnames(m0)[1] <- "seq_id"
...
q <- gggenomes(big_data, l0) +
  geom_seq() +
  geom_seq_label(size = 1.5) +
  scale_x_reverse() +
  geom_gene(aes(fill = matched), position = "strand", size = 2) +
  theme(axis.title.x = element_text(size = 10)) +
  labs(x = "Bases from Gene Start Site", size = 10) + 
  theme(plot.title = element_text(hjust = 0.5)) +
  ggtitle("sisA")
ggsave("degsisA2.png", q, width = 12, height = 30, dpi = 300)
```
...
c <- 1
for (n in x) {
  m0 <- read.table(file = paste("all_runt/5000_upstream/plus/canonical_tagteam/csv/", n, "_genes_plus_5000_upstream.csv", sep= ""), sep = '\t', header = TRUE)
  m0$seqID[0:length(m0$seqID)] <- n
  colnames(m0)[1] <- "seq_id"
  m0$start <- 5000 - m0$start + 8
  m0$end <- 5000 - m0$end - 8
  data.list[[c]] <- assign(paste0("plus", n), m0)
  c <- c + 1
}
#minus
for (n in x) {
  m0 <- read.table(file = paste("all_runt/5000_upstream/minus/canonical_tagteam/csv/", n, "_genes_minus_5000_upstream.csv", sep= ""), sep = '\t', header = TRUE)
  m0$seqID[0:length(m0$seqID)] <- n
  colnames(m0)[1] <- "seq_id"
  m0$start <- m0$start + 8
  m0$end <- m0$end - 8
...
big_data <- do.call(rbind, data.list)
l0 <- read.csv("/workdir/maa367/table5000.csv")
...
q <- gggenomes(big_data, l0) +
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  ggtitle("runt")
ggsave("runt_plot.png", q, width = 12, height = 30, dpi = 300)
```
